...
# Kontrollera om vi är i blog-mappen
is_blog = os.path.basename(current_dir) == "blog"

# Sökväg till styrdokumenten (uppdatera vid behov)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                logger.error(f"Maximalt antal försök ({MAX_RETRIES}) uppnått för titelgenerering. Använder originaltitel.")
...
def generate_reformatted_markdown(source_markdown: str, file_path: Path, timeout=API_TIMEOUT):
    file_size = len(source_markdown)
    logger.info(f"Filstorlek för '{file_path}': {file_size:,} tecken")
    ...
    suggested_slug = filename[len(date) + 1:] if date_match else filename
    is_readme = file_path.name.lower() == "readme.md"
    styrdokument_content = load_styrdokument()
    if existing_front_matter:
        front_matter_data = frontmatter.loads(existing_front_matter)
        original_title = front_matter_data.get('title', get_first_heading(body_content) or file_path.stem)
    else:
...
*   Generera en YAML-front matter (--- ... ---) med följande Docusaurus-nycklar för en README-fil:
    - `title`: Sätt till "README" eller baserat på H1-rubriken om den finns. Om titeln innehåller specialtecken, omge med dubbla citattecken (").
    - `description`: En kort sammanfattning (1-2 meningar) av innehållet, t.ex. "Översikt över projektet/mappen {mappnamn}." Om beskrivningen innehåller specialtecken, omge med dubbla citattecken.
    - `slug`: Sätt till "readme".
    - `tags`: Förslag på 2-5 relevanta taggar baserat på innehållet, t.ex. ["readme", "overview"].
"""
        format_requirements = """
**FORMATKRAV FÖR README-INNEHÅLL:**
*   Använd följande struktur för innehållet:
    - H1-rubrik: `# README`
    - H2-rubriker (##) för sektioner som "Översikt", "Syfte", "Användning", "Exempel", "Begränsningar", "Felsökning" (om tillämpligt).
    - Använd punktlistor (-) för att lista information i sektioner.
    - Använd kodblock (```) för kodexempel eller kommandon.
    - Använd `inline-kod` för filnamn, variabler, kommandon etc.
...
*   Generera en YAML-front matter (--- ... ---) med följande Docusaurus-nycklar för ett blogginlägg:
    ...
    - `slug`: Använd den nya titeln för en logisk URL: "{new_slug}". Använd INTE "/blog" i sluggen.
    - `authors`: Sätt till "joelkvarnsmyr" som standard.
    - `date`: Sätt till "{date}" (datumet från filnamnet).
...
"""
        ...
        front_matter_requirements = f"""
*   Generera en YAML-front matter (--- ... ---) med följande Docusaurus-nycklar för ett dokument:
    - `title`: Sätt till den nya titeln: "{new_title}". Om titeln innehåller specialtecken, omge med dubbla citattecken (").
    - `description`: En kort sammanfattning (1-2 meningar) av innehållet. Om beskrivningen innehåller specialtecken, omge med dubbla citattecken.
    - `slug`: Använd den nya titeln för en logisk URL: "{new_slug}". Använd INTE "/docs" i sluggen.
    - `sidebar_label`: En kort version av titeln för sidomenyn: "{new_title}". Om värdet innehåller specialtecken, omge med dubbla citattecken.
    - `sidebar_position`: Sätt till 10 som standard.
    - `tags`: Förslag på 2-5 relevanta taggar baserat på innehållet.
"""
        format_requirements = """
**FORMATKRAV FÖR INNEHÅLL:**
...
"""
    full_prompt = f"""
**ROLL:** AI-assistent som är expert på att strukturera teknisk dokumentation i Markdown för Docusaurus.
**UPPDRAG:** Ta följande innehåll och formatera om det till korrekt Markdown för att förbättra läsbarhet och struktur. **Behåll allt ursprungligt innehåll och dess innebörd**, men förbättra rubriker, listor, kodblock och styckesindelning. **Lägg till emojis på lämpliga ställen, men överdriv inte.** **Generera en komplett Docusaurus-kompatibel front matter.**
**STYRDOKUMENT:**
--- BEGIN STYRDOKUMENT ---
{styrdokument_content}
--- END STYRDOKUMENT ---
...
**FRONT MATTER-KRAV:**
{front_matter_requirements}
*   Om det redan finns en front matter ({'Ja' if existing_front_matter else 'Nej'}), uppdatera den med saknade nycklar.
**OUTPUT:**
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if response.parts:
                ...
                logger.info(f"Innehåll ({len(content):,} tecken) omformaterat från '{file_path}'.")
                return content
            else:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                logger.error(f"Maximalt antal försök ({MAX_RETRIES}) uppnått för '{file_path}'.")
                return None
    return None
...
def rename_file(file_path: Path, content: str) -> Path:
    ...
    if not front_matter:
        ...
        try:
            ...
            logger.error(f"Kunde inte parsa front matter i '{file_path}': {e}. Använder filnamnet.")
            title = file_path.stem
    date_match = re.match(r'(\d{4}-\d{2}-\d{2})', file_path.name)
    date = date_match.group(1) if date_match else "2025-01-01"
...
def main():
    ...
    if not files:
        ...
        sys.exit(0)
    logger.info(f"Hittade {len(files)} .md-filer i '{current_dir}' att reformatera.")
    file_mapping = {}
    reformatted_files = []
